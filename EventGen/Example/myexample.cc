...
// Constructor 
MyAnalysis::MyAnalysis(int imagesize)
{
    imagesize *= imagesize;
    MaxN = imagesize;
    ...
    if(fDebug) cout << "MyAnalysis::MyAnalysis Start " << endl;
    ftest = 0;
    fDebug = false;
    fOutName = "test.root";
    tool = new My();
    ...
    //                         xbins       y bins
    detector = new TH2D("", "", 125, -6.25, 6.25, 225, -11.25, 11.25);
    for(int i = 1; i <= 125; i++)
    {
        for (int j = 1; j <= 225; j++)
        {
    ...
    if(fDebug) cout << "MyAnalysis::MyAnalysis End " << endl;
}
...
// Begin method
void MyAnalysis::Begin()
{
   // Declare TTree
   tF = new TFile(fOutName.c_str(), "RECREATE");
   tT = new TTree("EventTree", "Event Tree for My");
    ...
    int pixels, float range)
{
    ...
    if(fDebug) cout << "MyAnalysis::AnalyzeEvent Event Number " << ievt << endl;
...
// declate branches
void MyAnalysis::DeclareBranches()
{
